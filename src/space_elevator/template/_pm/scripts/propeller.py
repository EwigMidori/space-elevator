...
def read_progress_paths(args: argparse.Namespace, worktrees: list[dict[str, str]]) -> list[Path]:
    ...
    paths = {args.repo / PM_DIR_NAME / "progress.json"}
    for item in worktrees:
        worktree_path = Path(item["worktree"])
        candidate = worktree_path / PM_DIR_NAME / "progress.json"
        if candidate.exists():
...
执行要求：
1. 先读 `_pm/AGENTS.md`、`_pm/progress.json`、`_pm/docs/agents/README.md`，以及当前活跃批次对应的 `.tmp/` 规格。
2. 严格遵守 `_pm/AGENTS.md` 中的 PDCA 多代理流程、分支/工作树约束、进度状态约束和评审门槛。
3. 只推进 `_pm/progress.json` 允许启动的下一个未完成阶段/任务；不要越过 phase gate。
...
6. 只有在真实评审达到要求后，才允许把 progress 项目标记为 done。
7. 禁止把任何已经是 done 的 phase 或 task 改回 todo、in_progress、blocked 或 ready_for_review。可以默认信任现有 done 状态是经过审核后成立的，除非你发现 progress 文件本身损坏到无法解释，此时只能记录异常，不能自行回退这些 done 项。
...
12. 尽可能持续推进，直到监控中的 `_pm/progress.json` 全部完成；如果遇到当前环境内无法自行解除的具体阻塞，清楚写明阻塞点后退出。
